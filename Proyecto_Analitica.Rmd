...
PARA ARMAR EL MODELO UTILIZAR EN VEZ DE FORWARD LA VERSON MIXTA, creo que es funcion bond??


PUSE ESTO DE DUMMYS ACA PORQUE LO NECESITO PARA EMPEZAR MI PARTE PERO PONGAN LO CUANDO LO USEN DEONDE IRIA. QUE CREO QUE SERIA EN LA DIVISION 2 (LA TUYA PAULA CREO)

```{r}

PrestamosUY$Sexo_dummy <- as.factor(PrestamosUY$Sexo)
PrestamosUY$Educacion_dummy <- as.factor(PrestamosUY$Educacion)
PrestamosUY$Estado_Civil_dummy <- as.factor(PrestamosUY$Estado_Civil)
...
set.seed(1111)   
train <- sample(nrow(PrestamosUY), nrow(PrestamosUY)*0.7) 
test <- (-train)

# Arbol de clasificacion con la base de training (falta completar, hay que poner las variables dummy)
par(mfrow = c(1, 1))
Arbol1 = rpart(Default ~ Ingreso + Disponible + Importe + Edad + Sexo_dummy + Educacion_dummy + Estado_Civil_dummy,
               PrestamosUY[train,], method = 'class')

#PREGUNTAR: PUEDE SER QUE NO NOS ESTE DANDO LAS VARIABLES DUMMIES, como que no las toma.
...
Reglas de decisión:
  ...
  PRIMERA REGLA:  
    En este caso si el importe en dólares del préstamos >= 16000.
      En 89% de los datos de TRAIN esta en el NODO 2, con una proporción de NO DEFAULT de 91% y una proporción de 9% SI DEFAULT.
    En el caso de que este importe sea > 16000
      El 11% de los datos de TRAIN esta en el NODO 3, lo cual predice SI DEFAULT. Con una proporción de SI DEFAULT de 100% y 0% en NO DEFAULT.
  ...
  TERCERA REGLA
    En el caso de que la linea de crédito disponible sea < 74000, el ingreso anual del titular sea >= 15000 y el importe del prestamo >= 16000.
      El 77% de los datos de TRAAIN esta en el NODO TERMINAL 8, predice NO DEFAULT, con una proporción de 95% NO DEFAULT y 5% en SI DEFAULT. 
